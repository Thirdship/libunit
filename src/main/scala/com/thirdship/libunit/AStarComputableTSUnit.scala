...
class AStarComputableTSUnit(val numerator: List[TSUnit] = List.empty[TSUnit],
  ...
  override def *(unit: TSUnit): TSUnit = unit match {
    ...
    case unit: AStarComputableTSUnit => new AStarComputableTSUnit(
    ...
    case unit: TSUnit => new AStarComputableTSUnit(numerator.::(unit), denominator).simplifyType
  }
  ...
  override def /(unit: TSUnit): TSUnit = unit match {
    ...
    case unit: TSUnit => new AStarComputableTSUnit(numerator, denominator.::(unit))
  }
  ...
  override def conversionFunction(unit: TSUnit): (Double) => Double = {
    ...
    unit match {
       ...
       */
      case u: AStarComputableTSUnit => if (u.simplified) {	//ensure the other unit is simplified first
  ...
  override def isConvertible(unit: TSUnit): Boolean = {
    ...
    unit match {
       ...
       */
      case u: AStarComputableTSUnit => if(u.simplified){ //the other unit needs to be simplified as well
  ...
  override def equalUnits(unit: TSUnit): Boolean = {
    ...
    unit match {
      ...
      case u: AStarComputableTSUnit => if(u.simplified){ equateUnitLists(u)} else equalUnits(u.simplifyType)
  ...
  override def inverse = new AStarComputableTSUnit(denominator, numerator, simplified, new ScalarTSUnit(1 / scalar.value))
  ...
  private[libunit] def simplifyType: TSUnit = {
    ...
    if (n_scalars.nonEmpty || d_scalars.nonEmpty) {
      ...
      return new AStarComputableTSUnit(n, d, scalar= s).simplifyType
    }
  ...
  private def removeExactDuplicates(): TSUnit ={
    ...
    if (duplicate != null) {
      ...
      new AStarComputableTSUnit(n, d, scalar = scalar).simplifyType
    } else
      this
  }
  ...
  private def simplifyConvertibleUnits(): TSUnit ={
    ...
    */
    if(theSame.nonEmpty){
      ...
      return new AStarComputableTSUnit(n, d, false, new ScalarTSUnit(scalar.value * s)).simplifyType
     ...
     */
    new AStarComputableTSUnit(numerator, denominator, true, scalar)
  }
  ...
  private def parseRecursive(str: String, currentUnitParser: UnitParser): Option[_ <: TSUnit] = {
    ...
    if(units.nonEmpty) {
      val out_unit = units.foldLeft[TSUnit](new ScalarTSUnit())( (out, in) => in match {
        case i: AStarComputableTSUnit => i * out
        case o: TSUnit => out * o
      })
