 ...
 */
final case class UnitValuePair(private val value: Double, private val unit: TSUnit) {
	 ...
	 * @return this * that
	 */
	def *(that: UnitValuePair) = new UnitValuePair(value * that.value, unit * that.unit).simplify
	 ...
	 */
	def +(that: UnitValuePair) = new UnitValuePair(value + that.convertTo(this).value, unit)
	 ...
	 * @return 1/this
	 */
	def inverse = new UnitValuePair(1/value, unit.inverse)
	 ...
	 * @throws UnitsException when unit conversion occurs but there is an error
	 */
	def convertTo(thatUnit: TSUnit): UnitValuePair = new UnitValuePair(unit.convert(thatUnit, value), thatUnit)
	 ...
	 * @throws UnitsException when unit conversion occurs but there is an error
	 */
	def convertTo(that: UnitValuePair): UnitValuePair = convertTo(that.unit)
	 ...
	 */
	 protected def simplify:UnitValuePair = unit match {
		...
		case u: ComputableTSUnit => if(u.scalar.value == 1)
			this else new UnitValuePair(value * u.scalar.value, new ComputableTSUnit(u.numerator, u.denominator))
		// If it as scalar, if it is not as simple as it can be, move that simplify
		case u: ScalarTSUnit => if(u.value == 1) this else new UnitValuePair(value * u.value, new ScalarTSUnit())
		case u: TSUnit => this
	}
}